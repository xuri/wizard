								...
								<div class="input-group col-md-12" style="margin:0 0 1em 0">
									<span class="input-group-btn" style="width: 20%; padding: 0 10px 0 0;">
										<select class="form-control input-sm" name="is_notify">
											<option value="0">{{ Lang::get('admin/users/noactive.not_notified') }}</option>
											<option value="all">{{ Lang::get('admin/users/noactive.all_status') }}</option>
											<option value="1" selected="true">{{ Lang::get('admin/users/noactive.has_notified') }}</option>
										</select>
									</span>
									...
									<tbody>
										@foreach($datas as $data)
											<?php
												...
												$has_notify = Like::where('is_notify', 1)->where('receiver_id', $data->receiver_id)->first();
											?>
												@if($passtime > 259200)
												<tr>
													<td>{{ $data->id }}</td>
